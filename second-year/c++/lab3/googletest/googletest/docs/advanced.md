...
```c++
template <typename typeName> class Foo {
 public:
  void Bar() { ::testing::StaticAssertTypeEq<int, typeName>(); }
};
```
...
initializeShips return object of type 'bool' with an rvalue of type 'void'"` or
...
class FooTest : public ::testing::Test {
 protected:
  ...
  static typeName* shared_resource_;
};

typeName* FooTest::shared_resource_ = nullptr;
...
must be derived from both `testing::Test` and `testing::WithParamInterface<typeName>`
(the latter is a pure interface), where `typeName` is the type of your parameter
values. For convenience, you can just derive the fixture class from
`testing::TestWithParam<typeName>`, which itself is derived from both `testing::Test`
and `testing::WithParamInterface<typeName>`. `typeName` can be any copyable type. If it's a
...
TEST_P(FooTest, DoesBlah) {
  ...
  // of the TestWithParam<typeName> class:
