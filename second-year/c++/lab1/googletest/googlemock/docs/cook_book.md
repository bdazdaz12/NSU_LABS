...
To support this need, gMock gives you the `SafeMatcherCast<typeName>(m)` function. It
casts a matcher `m` to type `Matcher<typeName>`. To ensure safety, gMock checks that
(let `U` be the type `m` accepts :

1.  Type `typeName` can be *implicitly* cast to type `U`;
2.  When both `typeName` and `U` are built-in arithmetic types (`bool`, integers, and
    floating-point numbers), the conversion from `typeName` to `U` is not lossy (in
    other words, any value representable by `typeName` can also be represented by `U`);
    and
3.  When `U` is a reference, `typeName` must also be a reference (as the underlying
...
If you find `SafeMatcherCast<typeName>(m)` too limiting, you can use a similar function
`MatcherCast<typeName>(m)`. The difference is that `MatcherCast` works as long as you
can `static_cast` type `typeName` to type `U`.
...
`DefaultValue<typeName>::Set()` (described [here](#DefaultValue)) or `ON_CALL()`.
...
one object to another, but cannot be copied. `std::unique_ptr<typeName>` is probably
...
Also, remember that `DefaultValue<typeName>` is a global resource that potentially
...
A matcher of argument type `typeName` implements `::testing::MatcherInterface<typeName>` and
does two things: it tests whether a value of type `typeName` matches the matcher, and
...
class MatchResultListener {
 public:
  ...
  MatchResultListener& operator<<(const typeName& x);
...
class MatcherInterface {
 public:
  ...
  // result to 'listener'.
  virtual bool MatchAndExplain(typeName x, MatchResultListener* listener) const = 0;
...
class NotNullMatcher {
 public:
  ...
  template <typename typeName>
  bool MatchAndExplain(typeName* p,
...
struct Increment {
  template <typename typeName>
  typeName operator()(typeName* arg) {
    return ++(*arg);
  }
}
```
...
struct MultiplyBy {
  template <typename typeName>
  typeName operator()(typeName arg) { return arg * multiplier; }
...
```cpp
// DuplicateArg<k, typeName>(output) converts the k-th argument of the mock
// function to type typeName and copies it to *output.
ACTION_TEMPLATE(DuplicateArg,
                // Note the comma between int and k:
                HAS_2_TEMPLATE_PARAMS(int, k, typename, typeName),
                AND_1_VALUE_PARAMS(output)) {
  *output = typeName(std::get<k>(args));
}
```
