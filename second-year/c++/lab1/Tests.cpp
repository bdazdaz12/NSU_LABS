...
TEST(MemoryAllocationTest, ShrinkMemoryFree) {
    ...
    EXPECT_EQ(expectedCapacity, set.capacity());
    EXPECT_EQ(secondBlockEndIdx + 1, set.size());
}
...
TEST(TritSetOperationsTest, OrTest) {
    ...
    b[2] = Trit::Unknown;
    TritSet c = a | b;
    EXPECT_EQ(c[0], Trit::True);
    EXPECT_EQ(c[1], Trit::Unknown);
    EXPECT_EQ(c[2], Trit::Unknown);
}
...
TEST(TritSetOperationsTest, NotTest) {
    ...
    a[2] = Trit::Unknown;
    TritSet b = !a;
    EXPECT_EQ(b[0], Trit::False);
    EXPECT_EQ(b[1], Trit::True);
    EXPECT_EQ(b[2], Trit::Unknown);
}
...
TEST(TritSetFunctionsTests, cardinalityMapTest) {
    ...
    EXPECT_EQ(cardinalityMap[Trit::False], 3);
    EXPECT_EQ(cardinalityMap[Trit::Unknown], 2);
    EXPECT_EQ(cardinalityMap[Trit::True], 1);
}
